...
function setupInvoiceHandlers() {
  ...
  ipcMain.handle("invoice:generatePDF", async (event, { transactionId, transactionType }) => {
    return await generateInvoicePDF(transactionId, transactionType);
  });

  // Print invoice to specific printer
  ipcMain.handle("invoice:print", async (event, { transactionId, transactionType, printerName, silent = false }) => {
    try {
      // Generate PDF first
