...
function verifyPassword(inputPassword, storedHash) {
  ...
  if (storedHash.startsWith('sha256:')) {
    ...
    return crypto.timingSafeEqual(Buffer.from(storedHash.slice(7), 'hex'), Buffer.from(hash, 'hex'));
  }
  return false;
}
...
function registerAuthIpc() {
  ...
  ipcMain.handle('auth:changePassword', (event, { userId, newPassword }) => {
    if (!newPassword || newPassword.length < 6) throw new Error('Password terlalu pendek');
    const hash = scryptHash(newPassword);
    getDb().prepare('UPDATE users SET password_hash = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').run(hash, userId);
    return true;
  });
}
