...
export default {
    ...
    setup() {
        ...
        const stats = computed(() => {
            ...
            const totalDeficit = alerts.value.reduce((sum, item) => sum + item.deficit, 0)
            const avgDeficit = totalDeficit
                ? (totalDeficit / alerts.value.length).toFixed(1)
                : 0
            const lowestStock = Math.min(...alerts.value.map((item) => item.stock_quantity || 0))
        ...
        const loadData = async () => {
            ...
            try {
                ...
                alerts.value = rows.map((item) => ({
                    ...item,
                    deficit: Math.max((item.min_stock_alert || 0) - (item.stock_quantity || 0), 0)
                }))
            } catch (err) {
