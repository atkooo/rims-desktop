  ...
  <div class="report-page">
    <div class="page-header">
      <div>
        <h1>Sewa Aktif</h1>
        <p class="subtitle">
          Monitoring transaksi sewa yang masih berjalan beserta potensi
          keterlambatan.
        </p>
      </div>
...
export default {
  ...
  setup() {
    ...
    const stats = computed(() => {
      ...
      const overdue = rentals.value.filter(
        (item) => (item.days_overdue || 0) > 0,
      );
      const overdueCount = overdue.length;
      const avgOverdueDays = overdueCount
        ? (
            overdue.reduce((sum, item) => sum + item.days_overdue, 0) /
            overdueCount
          ).toFixed(1)
        : 0;
