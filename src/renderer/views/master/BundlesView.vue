<template>
    <div class="data-page">
        <div class="page-header">
            <div>
                <h1>Data Paket</h1>
                <p class="subtitle">Informasi paket barang/aksesoris tanpa aksi perubahan.</p>
            </div>
...
export default {
    ...
    setup() {
        ...
        const stats = computed(() => {
            ...
            const activeBundles = bundles.value.filter((bundle) => bundle.is_active).length
            const totalStock = bundles.value.reduce(
                (sum, bundle) => sum + (bundle.stock_quantity || 0),
                0
            )
            const rentalBundles = bundles.value.filter((bundle) => bundle.bundle_type === 'rental').length
